...
class ChapterVideoBuilder:
    ...
    def rendered_segment_to_preview(
        ...
        duration = render_config.default_silent_clip_duration
    ...
    def _safe_output_name(self, name: str) -> str:
        ...
        if not cleaned:
            cleaned = "audio_visualizer_video.mp4"
    ...
    def _overlay_position(self, visualizer: d.AudioVisualizerConfig, width: int, height: int) -> tuple[int, int]:
        ...
        x_center = max(0, (width - visualizer.width) // 2)
        x_right = max(0, width - visualizer.width - visualizer.margin_x)
        y_top = visualizer.margin_y
        y_center = max(0, (height - visualizer.height) // 2)
        y_bottom = max(0, height - visualizer.height - visualizer.margin_y)
        positions = {
